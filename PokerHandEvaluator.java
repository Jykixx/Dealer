...
public class PokerHandEvaluator {
    ...
    public static int evaluateHand(List<Card> cards) { //оценка рукии
        cards.sort(Collections.reverseOrder());

        boolean flush = isFlush(cards);
        boolean straight = isStraight(cards);

        if (flush && straight && cards.getFirst().getRank().equals("A")) {
    ...
    private static boolean isThreeOfAKind(List<Card> cards, int n) { // сколько в списке карт олдного номинала
    ...
    private static Map<String, Integer> rankCounts(List<Card> cards) { // подсчет количества карт каждого номинала в списке карт
        Map<String, Integer> rankCount = new HashMap<>();
        for (Card card : cards) {
            rankCount.put(card.getRank(), rankCount.getOrDefault(card.getRank(), 0) + 1);
        }
    ...
    public static int compareHands(List<Card> hand1, List<Card> hand2) {
        ...
        // Если ранги равны, сравниваем старшие карты
